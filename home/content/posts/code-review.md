...
2. Now that we’re aligned on both this being the right problem to address and the right solution to that problem, **verify the change does as intended**. It’s not *that* uncommon for a PR to either solve a slightly different problem than claimed, or push forward an approach that is (subtlety, but significantly) different than described. You can consider t**his as a pass checking for *high-level correctness** (aka claim consistency)*.
